...
class QAModelV1(nn.Module):
    def __init__(self, vocab_size, embed_dim, lstm_units, max_context_len):
        ...
        self.lstm_context = nn.LSTM(embed_dim, lstm_units, bidirectional=True, batch_first=True)
        self.lstm_question = nn.LSTM(embed_dim, lstm_units, bidirectional=True, batch_first=True)
    ...
    def forward(self, context, question):
        ...
        question_rep = question_enc[:, -1, :].unsqueeze(1).expand(-1, self.max_context_len, -1)
...
class QAModelV2(nn.Module):
    def __init__(self, vocab_size, embed_dim, lstm_units, max_context_len):
        super().__init__()
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)
        self.lstm_context = nn.LSTM(embed_dim, lstm_units, bidirectional=True, batch_first=True, dropout=0.3)
        self.lstm_question = nn.LSTM(embed_dim, lstm_units, bidirectional=True, batch_first=True, dropout=0.3)
        self.attention = nn.MultiheadAttention(embed_dim=lstm_units * 2, num_heads=4, batch_first=True, dropout=0.1)
...
class QAModelV3(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_size, pad_idx=0):
        ...
        self.ctx_encoder = nn.LSTM(embed_dim, hidden_size, num_layers=2, bidirectional=True, batch_first=True, dropout=0.2)
        self.q_encoder = nn.LSTM(embed_dim, hidden_size, num_layers=2, bidirectional=True, batch_first=True, dropout=0.2)
        ...
        self.span_modeling = nn.LSTM(hidden_size * 2, hidden_size, bidirectional=True, batch_first=True)
    ...
    def forward(self, context_ids, question_ids):
        ...
        cls_ctx = modeled[:, 0, :]
        q_summary = q_enc[:, -1, :]
        no_answer_logit = self.no_answer_head(torch.cat([cls_ctx, q_summary], dim=-1)).squeeze(-1)
